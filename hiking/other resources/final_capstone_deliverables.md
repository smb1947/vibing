# AllTrails AI Copilot

## Deliverable 1: Problem Statement
AllTrails currently serves as the world’s leading trail directory. However, a critical gap exists when user looks for trails recommendations as Alltrails currently provides recommendations based on "static trail metadata" (distance, gain, location). Hikers, particularly in high-variability regions like **Washington State**, currently face a significant "research tax," spending 4-5 hours cross-referencing AllTrails with fragmented external sources like the National Weather Service (NWS) for weather, Northwest Avalanche Center (NWAC) for snow risk, WSDOT for road closures, and WTA for recent ground-truth conditions. This "tab fatigue" leads to app abandonment and, more critically, safety risks when users fail to identify hazardous conditions like sudden road washouts in the Cascades or high-elevation snow levels in the Olympics.

**Target Audience**
Recreational hikers and group organizers in **Washington State** who require high-confidence, safety-validated trail recommendations for specific dates.

**Quantitative Metrics of Success**
1. **Reduction in Planning Time:** Decrease the median time required for a user to select a safety-validated trail from **4-5 hours** (manual multi-site research) to **under 5 minutes** (automated agentic workflow), representing a ~90% improvement in planning efficiency.
2. **Accuracy Relative to Real Conditions:** Achieve a **95% data-alignment rate** for safety filters. A recommendation is "accurate" only if the agentic output matches official real-time signals for road closures (WSDOT) and trailhead snow depth (NWAC) on the morning of the hike.
3. **User Satisfaction with Recommendations:** Maintain a **CSAT (Customer Satisfaction Score) of 4.5/5.0** or higher for the "Why This Trail" generated reasoning. Success is defined by users selecting one of the Top 3 recommended picks for their weekly hike in >80% of sessions.
4. **Reduction in Impacted Hikes:** Aim for a **5% incidence rate** of "failed outings" due to foreseeable environmental hazards. Success is measured by the reduction of trips where users encounter significant unflagged obstacles, such as extreme weather (high wind/precip), high avalanche risk, closed access roads, or trail conditions requiring technical gear (e.g., microspikes/ice axe) that were not explicitly warned about by the agent.
...
**Data Sources**
The agentic workflow aggregates five distinct data streams, currently specialized for the **Washington State** region:
1. **AllTrails (Static):** Trail name, distance, elevation gain, difficulty, and unique URL.
2. **National Weather Service (NWS):** Point-specific weather forecasts (temperature, precipitation %, wind speed).
3. **Northwest Avalanche Center (NWAC):** Regional snow level forecasts and avalanche hazard ratings.
4. **Washington State Department of Transportation (WSDOT):** Highway and forest road closure status and chain requirements.
5. **Washington Trails Association (WTA):** Crowdsourced trip reports from the last 14 days to identify current trail surface conditions (e.g., "muddy," "icy," "clear").

**Geographic Scope:** While the architecture is scalable, the current prototype is strictly optimized for **Washington State**, utilizing region-specific data from the Northwest Avalanche Center and the Washington Trails Association to ensure maximum safety and accuracy for local Cascades and Olympics trails.

**Data Construction & Cleaning**
The system standardizes these disparate sources through a multi-step pipeline:
- **Recency Filtering:** Automatically discarding Washington Trails Association trip reports older than 14 days to ensure environmental data is "fresh."
- **Comparative Logic:** Comparing the `trailhead_elevation` (from AllTrails) against the `forecasted_snow_level` (from the Northwest Avalanche Center). If the trailhead is >500ft above the snow level, the trail is flagged for hard rejection.
- **Entity Extraction:** Using LLM-based sub-agents to parse unstructured Washington Trails Association reports to determine "sedan-friendliness" of access roads.

## Deliverable 3: Demo
MVP Code: [https://github.com/smb1947/vibing/blob/main/hiking/skill.md](https://github.com/smb1947/vibing/blob/main/hiking/skill.md)

## Deliverable 4: Executive Summary (PRFAQ)
